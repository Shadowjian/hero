...
export default function Navbar({
  ...
  const handleShowLoginForm = () => {
    setLoginState({ ...loginState, userName: "", email: "", password: "" })
    setShowLoginForm(!showLoginForm)
  }
  const handleShowJoinForm = () => {
    setJoinState({ ...joinState, userName: "", email: "", password: "" })
  ...
  const handleLogin = e => {
    ...
    if (!loginState.email) {
      ...
      const userFound = users.find(({ email }) => email === loginState.email)
      !userFound && setErrors({ ...errors, email: "email not found" })

      if (userFound.password !== loginState.password) {
  ...
  const searchGigs = e => {
    ...
    setGigs(
      gigs.filter(gig => {
        return gig.power.includes(searchInput)
      })
    )
  }
    ...
    <Dialog open={showJoinForm}>
      ...
      <DialogContent>
        <form onSubmit={handleJoin}>
          ...
          <TextField
            autoFocus
            error={joinState.password !== passwordConfirm}
            helperText={
              joinState.password !== passwordConfirm &&
              "Please confirm your password with the exact match"
            }
